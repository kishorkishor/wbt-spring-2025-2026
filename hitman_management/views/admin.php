...
<title>Admin Dashboard &mdash; Hitman Management</title>
...
<main class="main-content">
    <div class="page-header">
        <div>
            <h1 class="page-title">Manage Handlers</h1>
            <p class="page-sub">Register, edit, search and remove handler accounts</p>
        </div>
    </div>

    <?php if (isset($_GET['msg'])): ?>
        <?php $messages = ['added' => 'Handler added successfully.',
                           'updated' => 'Handler updated successfully.',
                           'deleted' => 'Handler deleted successfully.'];
    ...
    <div class="card form-card">
        <h3 class="card-title">
            <?= $isEdit ? '&#9998; Edit Handler (#' . intval($editing['id']) . ')' : '+ Add New Handler' ?>
        </h3>
        <form method="POST"
            ...
            <div class="field-row">
                <div class="field">
                    <label for="name">Handler Name</label>
            ...
            <div class="form-actions">
                ...
                <?php else: ?>
                    <button type="submit" class="btn btn-primary">Save Handler</button>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="card">
        <div class="card-toolbar">
            ...
            <span class="badge" id="resultCount"><?= count($handlers) ?> total</span>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody id="tableBody">
                    <?php if (empty($handlers)): ?>
                        <tr><td colspan="5" class="empty">No handlers yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($handlers as $i => $handler): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= htmlspecialchars($handler['name']) ?></td>
                                <td><?= htmlspecialchars($handler['contact']) ?></td>
                                <td><?= htmlspecialchars($handler['username']) ?></td>
                                <td class="text-right">
                                    <a class="btn-sm btn-edit"
                                       href="index.php?page=admin&action=edit&id=<?= $handler['id'] ?>">Edit</a>
                                    <a class="btn-sm btn-delete"
                                       href="index.php?page=admin&action=delete&id=<?= $handler['id'] ?>"
                                       onclick="return confirm('Delete this handler?')">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
(function () {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
            fetch('index.php?page=ajax&type=handler&q=' + encodeURIComponent(input.value.trim()),
