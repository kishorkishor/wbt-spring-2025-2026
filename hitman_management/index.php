...
/* ------------- AJAX search endpoint -------------
   ...
   ?page=ajax&type=handler&q=...    (admin only)
   ?page=ajax&type=contract&q=...   (handler only)
*/
if ($page === 'ajax') {
    ...
    if ($type === 'handler' && $_SESSION['user']['role'] === 'admin') {
        echo json_encode($q === '' ? getHandlers($conn) : searchHandlers($conn, $q));
    } elseif ($type === 'contract' && $_SESSION['user']['role'] === 'handler') {
        echo json_encode($q === '' ? getContracts($conn) : searchContracts($conn, $q));
    } else {
        http_response_code(403);
...
if ($page === 'handler' && $_SESSION['user']['role'] !== 'handler') { header('Location: index.php?page=login'); exit; }
...
switch ($page) {
    ...
    case 'handler':  handlerCtrl($conn);  break;
    default:
        header('Location: index.php?page=login');
        exit;
}
